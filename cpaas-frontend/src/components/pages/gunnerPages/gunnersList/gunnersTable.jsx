...
class GunnersTable extends Component {
  getGradeFromUrl = () => {
  ...
  fetchPage = (page, limit = this.props.pagination?.limit || 10) => {
    ...
    this.props.getAgniveers(
      ...
      params.get("course"),
      params.get("armyNo"),
      params.get("tradeId") || getTradeId("Gunner"),
      gradeValue,
      this.props.text,
    );
  };
  ...
  render() {
    ...
    return (
      <TradeUsersTable
        trade="gunner"
        tradeId={getTradeId("Gunner")}
        grade={grade}
        data={agniveers}
        pagination={pagination}
        loading={loadingAgniveers}
        fetchPage={this.fetchPage}
        addPathPrefix="/gunner/gunners/basic-info"
        viewPathPrefix="/gunner/gunners"
        history={history}
        location={location}
      />
    );
...
const mapStateToProps = (state) => (
  console.log("redux state is", state.search?.searchValue),
  {
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(GunnersTable),
);
