...
class OperatorHomeContainer extends Component {
  componentDidMount() {
    const tradeId = getTradeId(this.props.activeTrade);

    this.props.fetchCourses(tradeId);   // fetch courses on load
  }

  componentDidUpdate(prevProps) {
    if (prevProps.activeTrade !== this.props.activeTrade) {
      const tradeId = getTradeId(this.props.activeTrade);
      this.props.fetchCourses(tradeId); // refetch if trade changes
    }
  }
